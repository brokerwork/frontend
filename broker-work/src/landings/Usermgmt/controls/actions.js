...
import { DISABLED_FIELD_NORMAL, COMMISSION_FIELD_KEYS } from '../constant';
import { citdField } from 'utils/ctidConstant';
// ---------------------------------------------
// action typs
// ---------------------------------------------
const PRE_FIX = 'USERMGMT_';
...
export const VALIDATE_USER_ID_NUM_IN_TYPE_5 = `${PRE_FIX}VALIDATE_USER_ID_NUM_IN_TYPE_5`;
export const GET_AUTH_SETTING = `${PRE_FIX}GET_AUTH_SETTING`;
export const MODIFY_AUTH_STATUS = `${PRE_FIX}MODIFY_AUTH_STATUS`;
export const GET_DEFAULT_PARAMS = `${PRE_FIX}GET_DEFAULT_PARAMS`;
...
export const saveTransferUsers = createAction(
  ...
  (superiorId, userIds) =>
    post({
      url: `/v2/user/${superiorId}/updateParentBatch`,
      data: userIds
    })
);
      ...
      res.data.forEach(item => {
        ...
        if (item.values) {
          ...
          item.values.forEach(itm => {
            option.push({
              label: itm.comment ? `${itm.value}/(${itm.comment})` : itm.value,
              value: itm.detailId
            });
          });
 ...
 * 用户身份证判重
 */
export const validateUserIdNum = createAction(
  VALIDATE_USER_ID_NUM_IN_TYPE_5,
  (idNum, excludeUserId) => {
    if (!idNum) {
      //身份号为空时不需要验重
    ...
    return get({
      url: `/v1/user/exist?key=idNum&value=${idNum}&excludeUserId=${excludeUserId ||
        ''}`
    });
  }
);
...
// 获取BW用户2FA设置
export const getAuthSetting = createAction(GET_AUTH_SETTING, () =>
  get({
    url: '/v1/user/login/access'
  })
);

export const modifyAuthStatus = createAction(MODIFY_AUTH_STATUS, pubUserId =>
  post({
    url: `/v1/user/2fa/disable/${pubUserId}`
  })
);
// 获取默认参数

export const getDefaultParams = createAction(GET_DEFAULT_PARAMS, () =>
  get({
    url: '/v1/report/setting/level/default/params'
  })
);
