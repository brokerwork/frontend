...
const parseLevelData = LevelList => {
  let copyData = LevelList.concat();
  copyData = copyData.map(level => {
    return {
      ...level,
      label: level.name,
      value: `${level.id}`
...
export const authSetting = handleActions(
  {
    [GET_AUTH_SETTING]: (state, { type, payload = {} }) => {
      return _.get(payload, 'twoFAConfig.enable', false);
    }
  },
  false
);
export const defaultParams = handleActions(
  {
    [GET_DEFAULT_PARAMS]: (state, { payload }) => payload
  },
  []
);
