...
import { Card, Message, Popover, Icon } from 'lean-ui';
import { injectExistIdNumWarning } from './IdInfoFormControl';
...
export default class UpdateUserPanel extends PureComponent {
  ...
  submitSuccess(field, data) {
    ...
    if (Object.keys(this.__submitData).length === formLength) {
      ...
      handledData.accountType = 'Commission';
  ...
  submitDataHandle = data => {
    ...
    if (data['reakrule']) {
      ...
      finallyData['parent'] =
        originReakData['parent'] === 'null' ? '' : originReakData['parent'];
      finallyData['levelId'] =
        originReakData['levelId'] === 'null' ? '' : originReakData['levelId'];
      finallyData['levelName'] =
        originReakData['levelId'] === 'null' ? '' : originReakData['levelName'];
    ...
    if (finallyData.entityNo) {
      finallyData.entityNo = finallyData.entityNo.trim();
    }
    return finallyData;
  };
  isRoleEdit = () => {
  ...
  formatterFields = () => {
    ...
    console.log('basicFormColumns', basicFormColumns);
    const rights = getRights(relationUserInfo, userRights, isTask);
    ...
    const filterData = copyData.filter(el => {
      return el.key.indexOf('points') === -1;
    });
    let newCopyData = this.formatSensitiveFields(filterData);
      ...
      newCopyData[entityNoIdx] = {
        ...
        tip: (
          <Popover
            ...
            trigger="click"
            content={
              <div className={cs.tips_content}>
                {i18n['usermgmt.usercard.userinfo.entityNo.tips']}
              </div>
            }
          >
        ...
        validate: value => {
          if (value === '0') {
            return i18n['usermgmt.usercard.userinfo.entityNo.error'];
          } else {
            return undefined;
          }
        },
        validateType: type === 'edit' && { required: true },
        placeHolder: type === 'add' && 'usermgmt.usercard.id_placeholder'
      };
    }
  ...
  getRequireData = () => {
    ...
    getFormColumns(isTask).then(res => {
      if (res.result && !!res.data) {
        //初始时调用一次身份证验重
        const { t_user_agent } = res.data;
        if (t_user_agent) {
          if (
            t_user_agent.some(item => item.key === 'idType') &&
            editUserInfo['idType'] === '5'
          ) {
            this.iDInfoValidate(editUserInfo, null, null, 'idNum');
          }
        }
      }
    });
  ...
  iDInfoValidate = (values, ...p) => {
    // p[2]里面存的是字段名
    // 这里只验证身份证
    if (p[2] !== 'idNum') return Promise.resolve();
    const { idType, idNum } = values;
    const { moreFormColumns, editUserInfo } = this.props;
    // 调用ref中的方法，并传入校验值
    if (this.idNumRef) {
      this.idNumRef.onFormParamChange({
        idTypeIs5:
          moreFormColumns.some(item => item.key == 'idType') && idType == 5,
        idType,
        idNum,
        excludeUserId: editUserInfo.id
      });
    }
    // 不抛出错误
  ...
  render() {
    ...
    this.fields = fields;
    const moreFields = this.formatSensitiveFields(
      injectExistIdNumWarning(
        moreFormColumns,
        input => {
          this.idNumRef = input;
        },
