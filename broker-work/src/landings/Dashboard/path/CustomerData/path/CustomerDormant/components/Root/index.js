...
const addWorkbenceArr = ['NEW_CUSTOMER_TENANT_DORMANT_PAGE'];
...
export default class Root extends PureComponent {
  ...
  componentDidMount() {
    ...
    const type = 'NEW_CUSTOMER_DORMANT_PANEL';
    const params = {
      ...searchParams,
      type,
      fromTime: moment()
  ...
  render() {
    ...
    const tipsConfig = [
      {
        text: i18n['dashboard.tips.customer_dormant.tips1'],
    ...
    return (
      <div className={cs['container']} data-test="container">
        <div className={cs['navigation-bar']}>
          <div className={cs['navigation-left']}>
            <span className={cs['panel-title']}>
              {i18n['dashboard.navigator.customer_data.customer_dormant']}
            </span>
            {tipsConfig.length && (
              <Popover
        ...
        <PagePanel className={cs['table-panel']}>
          <PagePanel.Header>
            {i18n['dashboard.title.table.customer_dormant']}
          </PagePanel.Header>
          <PagePanel.Body className={cs['table-panel-body']}>
            <Table>
              <Table.Header>
                ...
                <th>
                  {i18n['dashboard.data_type.NEW_CUSTOMER_TENANT_DORMANT_PAGE']}
                </th>
              </Table.Header>
              <Table.Body>
                {tableData.map(item => {
                  return (
                    <tr key={item.date}>
                      <td>{item.date}</td>
                      <td>{item.NEW_CUSTOMER_TENANT_DORMANT_PAGE}</td>
                    </tr>
                  );
                })}
              </Table.Body>
