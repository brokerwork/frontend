...
const ReakRuleForm = reduxForm({
  ...
  asyncValidate: (values = {}, dispatch, props) => {
    ...
    if (
      ...
      const { parent } = values;
      return get({
        url: `/v1/user/task/${values.parent}/simple`
      }).then(({ result, data }) => {
        ...
        if (values[`${item.ruleId}`] > item.maxCommissionValue) {
          errors[`${item.ruleId}`] = (
            <FormattedMessage
              id="custom_field.required"
              defaultMessage={`${i18n[
                'usermgmt.usercard.reak_input_alert'
              ]}{value}`}
...
export default class RuleForm extends Component {
  ...
  render() {
    const { showRuleDetailModal, dataReady, levelChanged } = this.state;
    const {
      ruleDetail,
      upwardInitvalue,
      type,
