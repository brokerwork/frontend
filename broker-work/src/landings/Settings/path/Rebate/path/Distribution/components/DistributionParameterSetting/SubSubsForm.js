...
export const SUB_SUBS_FORM = 'DISTRIBUTION_SUB_SUBS_FORM';
...
const sumbit = (subSubsList, values) => {
  const errors = {};
  ...
  if (required(values.subSubType)) {
    errors.subSubType = required(values.subSubType);
  }
  ...
  if (values.commissionType == 3 && values.value) {
    ...
    subSubsList.filter(item => item.commissionType == 3).forEach(item => {
...
class SubSubsForm extends PureComponent {
  render() {
    const {
      handleSubmit,
      relationList,
    ...
    return (
      <Form
        onSubmit={handleSubmit(sumbit.bind(this, subSubsList))}
