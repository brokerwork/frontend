...
const { Td, Th } = UiTable;
import { RETURN_OPERATE_HEADER } from '../../constant';
...
export default class OperateRule extends PureComponent {
  ...
  selectMt4Groups = (server, keys, selected) => {
    const { data } = this.state;
    const copyData = JSON.parse(JSON.stringify(data));
          ...
          <Form.Control className={mt4GroupsError ? 'has-error' : ''}>
            {serverGroupList.length === 1 ? (
              <Picklist
                ...
                defaultSelectedKeys={[
                  copyData.mt4Groups[serverGroupList[0].serverId].value
                ]}
                placeholder={i18n['settings.rebate_setting.all_group']}
                disabled={
                  serverGroupList[0].enable === false ||
                  parseInt(copyData.mt4Groups[serverGroupList[0].serverId]) ===
                    -1
                }
