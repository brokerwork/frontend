        ...
        {show ? (
          <ul className={cs['export-list']} data-test="export-list">
            <li onClick={() => this.setState({ show: false })}>
              <a
                href={`/api/v2/account/import/excel/withdraw/${depositId}/export?onlyFail=false&token=${getToken()}`}
                target="_blank"
              >
                {i18n['account.batch_widthdraw.detail.export.origin_data']}
              </a>
            </li>
            <li onClick={() => this.setState({ show: false })}>
              <a
                href={`/api/v2/account/import/excel/withdraw/${depositId}/export?onlyFail=true&token=${getToken()}`}
                target="_blank"
              >
                {i18n['account.batch_widthdraw.detail.export.fail_record']}
              </a>
            </li>
          </ul>
        ) : (
