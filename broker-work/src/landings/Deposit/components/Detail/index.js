    ...
    return (
      <Modal show bsSize="lg" onHide={onHide}>
        <Modal.Header closeButton>
          <Modal.Title>{i18n['account.batch_deposit.detail']}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Table>
            <Table.Header>
              ...
              <th>{i18n['account.batch_deposit.preview.amount']}</th>
              <th>{i18n['account.batch_deposit.preview.comment']}</th>
              <th>{i18n['account.batch_deposit.detail.status']}</th>
            </Table.Header>
            <Table.Body>
                ...
                return (
                  <tr key={idx}>
                    ...
                    <td className={isFail ? 'text-danger' : ''}>
                      {isFail ? (
                        <FormattedMessage
                          id="account.batch_deposit.detail.status.fail"
                          defaultMessage={
                            i18n['account.batch_deposit.detail.status.fail']
                          }
                          values={{
                        ...
                        i18n['account.batch_deposit.detail.status.success']
                      )}
                    </td>
                  </tr>
                );
