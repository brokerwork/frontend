  ...
  render() {
    ...
    return (
      <div>
        <Table>
          <Table.Header>
            <th>{i18n['account.batch_deposit.detail.import_time']}</th>
            <th>{i18n['account.batch_deposit.detail.user']}</th>
            <th>{i18n['account.batch_deposit.detail.import_amount']}</th>
            <th>
              {i18n['account.batch_deposit.detail.import_success_amount']}
            </th>
            <th>{i18n['account.batch_deposit.detail.import_fail_amount']}</th>
            ...
            {depositList.map((item, idx) => {
              ...
              return (
                <tr key={idx}>
                  ...
                  <td>
                    {isImporting
                      ? i18n['account.batch_deposit.detail.handling']
                      : item.successCount}
                  </td>
                  <td>
                    {isImporting
