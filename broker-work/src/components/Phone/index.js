...
export default class Phone extends PureComponent {
  ...
  updateStateByProps = props => {
    ...
    const countryCode = (options.find(item => item.value == code) ||
      options[0] ||
      {}
    ).value;
    const initValue = {
      countryCode:
        !value.phone && !value.countryCode
          ...
          : value.phone && !value.countryCode ? '-1' : value.countryCode,
      phone: value.phone || ''
    };
  ...
  render() {
    ...
    return (
      <div
        className={`phone-field ${cs['container']} ${className} ${isSelected
          ? cs['isSelected']
          : ''} ${error ? cs['error'] : ''}`}
      >
        <DropdownForCode
