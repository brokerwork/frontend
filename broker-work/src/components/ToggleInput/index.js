...
export default class ToggleInput extends Component {
  ...
  fetchInputValue = props => {
    const { inputValue, onInput } = this.state;
    const { value } = props;
    if (inputValue !== value) {
      this.setState(
        {
  ...
  renderComponent = () => {
    ...
    const { inputValue } = this.state;
    const classStr = `${inputClassName} ${cs['input']} ${error
      ? cs['error']
      : ''}`;
    switch (inputType) {
      case 'number':
  ...
  render() {
    ...
    const cancelButtonText = cancelButton
      ? cancelButton === true ? i18n['general.cancel'] : cancelButton
      : undefined;
    return (
      <label
        className={`${cs['toggle-input-box']} ${inline
          ? cs['inline']
          : ''} ${staticIcon ? cs['static-icon'] : ''} ${className}`}
      >
        {focus ? (
          ...
          <span onClick={this.onFocus} data-test="label-group">
            ...
            {disabled ? (
              ...
              <i data-test="edit-icon" className={icon} />
            )}
          </span>
        )}
      </label>
