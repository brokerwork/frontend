...
import { Checkbox, Popover, Icon } from 'lean-ui';
// 示例
// <SelectTree
      ...
      return (
        <li key={index} data-value={item.entityNo}>
          {hasChildren ? (
            ...
            // />
            <Icon
              icon={`${!node['foldStatus'] ? 'caret-right' : 'caret-bottom'}`}
              onClick={toggleFoldStatus.bind(this, item.id)}
              className={cs.icon}
            />
          ) : (
          ...
          {item.tips && (
            ...
            >
              <Icon
                icon="question"
                className={cs['icon']}
                onClick={onTipsClick.bind(this, item.entityNo)}
              />
            </Popover>
          )}
          {unsafeMap[item.id] ? (
