...
function mask(dispatch) {
  console.log('aaa', openTimerCount, closeTimerCount, openTimer, closeTimer);
  if (openTimerCount > 1) {
...
export default function loadingMiddleware({ dispatch }) {
  return next => action => {
    ...
    if (isPromise(payload) && !(meta && meta.noMask)) {
      mask(dispatch);
      payload.then(result => unmask(dispatch), error => unmask(dispatch));
    }
    return next(action);
  };
}
