...
export default class Template extends PureComponent {
  ...
  onUpdateTemplateEmail = () => {
    const { getTemplateList, activeKey } = this.props;

    getTemplateList(activeKey).then(({ result }) => {
      if (result) {
  ...
  batchResetEmailTemplates = () => {
    const { batchResetEmailTemplates, selectedTemplate, getTemplateList, showTopAlert, activeKey } = this.props;
  ...
  render() {
    ...
    // 如果没有查询到设置的语言，则显示简体中文
    const defaultLanguage = languages.find(item => item.value === emailDefaultLanguage) || languages[0];
      ...
      <div>
        ...
        <AnimationWrapper>
          {showUpdateTemplateModal ? (
            <UpdateTemplate
              template={template}
              onClose={this.toggleModal.bind(this, 'UpdateTemplate', false)}
              activeKey={activeKey}
            />
          ) : (
            undefined
