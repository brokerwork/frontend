...
import {Row, Col, Button,
    ...
    ButtonGroup, Modal, SearchBox,Panel,NewSelect,Pagination,Table,Message,LoadingMask
} from 'fooui';
import {FailureFactor} from './failurefactor';
...
return [
    ...
    //},
    {
        title: I18nLoader.get('message.recipient'),
        dataIndex: 'receive',
        key: 'receive',
        width: '12%',
    ...
    },
    {
        ...
        width: '10%',
        renderer(value, message, index){
            var sendDate = getValue(message, 'sendDate');
            return sendDate
    ...
    {
        title: I18nLoader.get('message.status'),
        dataIndex: 'status',
        key: 'status',
        width: '10%',
        renderer(value, message, index,table){
            var receiveBox = table.props.owner;
            var status = getValue(message, 'status');
            if(status==='STATUS_SUCCESS'){
                return I18nLoader.get('message.send_success');
            }
            else if(status==='STATUS_FAIL'){
                return <a style={{textDecoration:'underline',cursor:'pointer'}} className="send-fail" onClick={()=>{receiveBox.failureFactor(message.error)}}>{I18nLoader.get('message.send_fail')}</a>
            }

        }

    },
    {   title: I18nLoader.get('message.operation'),
        dataIndex: 'operation',
        key: 'operation',
        width: '10%',
        renderer(value, message, index,table){
            ...
            var status = getValue(message, 'status');
            if(status==='STATUS_SUCCESS'){
                return <Button bsClass="btn btn-default fa fa-repeat" ></Button>
            }
            else if(status==='STATUS_FAIL'){
                return <Button bsClass="btn btn-primary fa fa-repeat" onClick={()=>{receiveBox.resendMessage(message.id)}}></Button>
            }else {
                return <Button bsClass="btn btn-default">{I18nLoader.get('message.processing')}</Button>
            }
...
class SendBox extends React.Component<P,S>{
    constructor(props) {
        super(props);
        this.state={
            totalPages:1,
...
//失败原因
    failureFactor = (messageerror) => {
        let self = this;
        let refContentCreator = function(){
            return <FailureFactor ref="failureFactor" errorcontent={messageerror}/>
        };
        let m:any = Modal.show({
            title: I18nLoader.get('message.failure_reason'),
            hasOk: false,
            hasCancel: false,
            hasReturn:true,
            onCancel: ()=>{},
    ...
    resendMessage =(messageid)=>{
        let self = this;
        let refContentCreator = function(){
            return <ResendMessage ref="resendMessage"/>
        };
        let m:any = Modal.show({
            title: I18nLoader.get('message.resend'),
            hasOk: true,
            hasCancel: true,
            onOk: (m) => {
                var configId = m.refs.resendMessage.refs.selectEmail.value;
                
                this.retry(messageid,configId).then(res=>{
                    if (res.result){
                        ...
                        Message.error(I18nLoader.get('message.send_success'));
                        m.close();
                    }
                });
            },
    ...
    //重新发送
    retry = (messageid,configId)=>{
        var url = '/v1/message/retry';
        var params = {
            "id": [messageid],
            "configId":configId
        }
        return HttpClient.doPost(url,params);
    }
    clearValue =()=>{
    ...
    render(){
        return (
            <div className="sendBox">
                <Panel title={I18nLoader.get('message.outbox')}>
