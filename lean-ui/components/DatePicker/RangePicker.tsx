...
import * as RangeCalendar from "rc-calendar/lib/RangeCalendar";
import * as RcDatePicker from "rc-calendar/lib/Picker";
...
export interface RangePickerState {
  ...
  rangesHoverValue: RangePickerValue | null;
}
...
> {
  ...
  handleRangeClick = (value: RangePickerPresetRange) => {
    this.handleRangeMouseLeave(); // 修复点击后未触发leave事件，从而未清空rangesHoverValue的值导致点击切换年月没有效果；
  ...
  handleRangesHover = (v: RangePickerValue) => {
    this.setState({
      rangesHoverValue: v,
      hoverValue: v
    });
    ...
    const operations = Object.keys(ranges || {}).map(range => {
      ...
      return (
        <a
          ...
          onMouseEnter={this.handleRangesHover.bind(this, value)}
  ...
  render() {
    ...
    const { value, showDate, hoverValue, open, rangesHoverValue } = state;
    const {
      prefixCls,
      popupStyle,
      style,
    ...
    fixLocale(rangesHoverValue, localeCode);
    ...
    const calendar = (
      <RangeCalendar
        ...
        value={rangesHoverValue || showDate}
