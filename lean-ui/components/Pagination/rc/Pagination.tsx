...
export interface PaginationProps {
  ...
  selectComponentClass?: React.ReactNode;
}
...
> {
  ...
  calculatePage = function(p?: number) {
    let pageSize = p;
  ...
  handleChange = (p: number) => {
    ...
    if (this.isValid(page)) {
      ...
      } else if (this.props.current === page) {
        return page;
      }
  ...
  runIfEnter = (...args: any[]) => {
    ...
    let restParams, _len, _key;
    for (
      _len = args.length, restParams = Array(_len > 2 ? _len - 2 : 0), _key = 2;
      _key < _len;
      _key++
    ) {
      restParams[_key - 2] = args[_key];
    }
  ...
  render() {
      ...
      return (
        ...
        >
          <li
            ...
            onClick={this.prev}
            data-tab-index={this.hasPrev() ? 0 : null}
    ...
    if (allPages <= 5 + pageBufferSize * 2) {
      for (let i = 1; i <= allPages; i++) {
        ...
        let className = "";
        if (i === 1) {
          className += ` ${prefixCls}-item-start`;
        }
        if (i === allPages) {
          className += ` ${prefixCls}-item-end`;
        }
        pagerList.push(
          <Pager
            locale={locale}
      ...
      lastPager = (
        <Pager
          ...
          active={false}
          className={`${prefixCls}-item-end ${
            right !== allPages - 1 ? prefixCls + "-item-start" : ""
          }`}
          showTitle={props.showTitle}
          itemRender={props.itemRender}
        />
      ...
      for (let i = left; i <= right; i++) {
        ...
        const className =
          i === left && left !== 2
            ? `${prefixCls}-item-start`
            : i === right && right !== allPages - 1
              ? `${prefixCls}-item-end`
              : undefined;
        pagerList.push(
          <Pager
      ...
      if (
        ...
        pagerList[pagerList.length - 1] = React.cloneElement(
          ...
          {
            className: `${
              pagerList[pagerList.length - 1].props.className
    ...
    return (
      ...
      >
        ...
        <li
          title={props.showTitle ? locale.prev_page : null}
          onClick={this.prev}
          data-tab-index={prevDisabled ? null : 0}
          onKeyPress={this.runIfEnterPrev}
          className={`${
        ...
        <li
          title={props.showTitle ? locale.next_page : null}
          onClick={this.next}
          data-tab-index={nextDisabled ? null : 0}
          onKeyPress={this.runIfEnterNext}
          className={`${
