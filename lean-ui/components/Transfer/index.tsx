...
export default class Transfer extends React.Component<TransferProps, any> {
  ...
  static propTypes = {
    ...
    transferLocale: PropTypes.object,
  ...
  onSort = (data: SortIndex, direction: TransferDirection) => {
    const { targetKeys = [], dataSource = [], onChange } = this.props;
    // move items to target box
    const oldIndex = data.oldIndex;
    const newIndex = data.newIndex;
    this.setState({
      [this.getSelectedKeysName('left')]: [],
    });
    this.handleSelectChange('left', []);
  ...
  render() {
    ...
    const cls = classNames(className, `${prefixCls}-transfer`);

    const titles = this.getTitles(transferLocale);
    return (
      <div className={cls}>
        <List
          prefixCls={`${prefixCls}-transfer-list`}
          titleText={titles[0]}
          ...
          itemUnit={transferLocale.itemUnit}
          itemsUnit={transferLocale.itemsUnit}
          body={body}
          footer={footer}
          lazy={lazy}
          sortable={false}
        ...
        />
        <Operation
          className={`${prefixCls}-transfer-operation`}
