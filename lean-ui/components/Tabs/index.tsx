...
import * as ScrollableInkTabBar from 'rc-tabs/lib/ScrollableInkTabBar';
import * as TabContent from 'rc-tabs/lib/TabContent';
...
export default class Tabs extends React.Component<TabsProps, any> {
  ...
  render() {
    ...
    if (type === 'editable-card') {
      ...
      React.Children.forEach(children as React.ReactNode, (child: React.ReactElement<any>, index) => {
        ...
        const closeIcon = closable ? (
           <Icon
             icon="close"
             onClick={e => this.removeTab.bind(this, child.key as string, e)}
           />
        ) : null;
        childrenWithClose.push(React.cloneElement(child, {
          tab: (
            <div className={closable ? undefined : `${prefixCls}-tabs-tab-unclosable`}>
              {child.props.tab}
              {closeIcon}
            </div>
          ),
      ...
      if (!hideAdd) {
        tabBarExtraContent = (
          <span>
            <Icon icon="plus" className={`${prefixCls}-tabs-new-tab`} onClick={this.createNewTab} />
            {tabBarExtraContent}
          </span>
        );
      }
    ...
    tabBarExtraContent = tabBarExtraContent ? (
      <div className={`${prefixCls}-tabs-extra-content`}>
        {tabBarExtraContent}
      </div>
    ) : null;
