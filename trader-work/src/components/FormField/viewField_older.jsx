...
class StepField extends Component {
  ...
  renderRow(stepData, item) {
    ...
    return (
      <div className={rowClassName}>
        <div className={`step-row-label ${labelClassName}`} title={item.label}>
          {item.label}
        </div>
        <div className={`step-row-value ${valueClassName}`}>
          {stepData[item.key]}
        </div>
      </div>
    );
        ...
        if (this.props.showChanged) {
          ...
          if (!item.relation) {
            elementArray = elementArray.concat(
              this.renderRelation(fieldsData, stepData, item)
            );
          }
        }
      });
  ...
  render() {
    ...
    let stepClassName = classNames("step-panel-old", className);
    return (
        ...
        <div className="step-body">
                ...
                return (
                  <div className="step-field" key={index}>
                    <div className="step-title">
                      {item.languageSettingMap &&
                        item.languageSettingMap[getType()]}
                    </div>
                    <div className="step-data">
                      {this.renderField(item.fieldList)}
                    </div>
                  </div>
                );
              })
          ...
          {fieldsData.description && (
            <div className="step-description">
              <div className="step-description-tit">
                {i18n["realaccount.homony.comment"]}
              </div>
              <div className="step-description-con">
                {fieldsData.description}
              </div>
            </div>
          )}
